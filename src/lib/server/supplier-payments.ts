 ...
 */
export async function rederiveDueDatesFifo(adminClient: AdminClient, invoiceId: string): Promise<void> {
  if (!invoiceId) return
  ...
  if (idsToMarkPaid.length > 0) {
    await adminClient
      .from('ap_supplier_invoice_due_dates')
      .update({ is_paid: true, paid_at: new Date().toISOString().slice(0, 10), payment_method: 'transfer' })
      .in('id', idsToMarkPaid)
  }
