            ...
            {isLoading ? (
              ...
              return (
                <TableRow key={m.id}>
                  ...
                  <TableCell><Badge variant="outline" className="text-xs gap-1"><Icon className={`h-3 w-3 ${isPositive ? 'text-green-600' : 'text-red-600'}`} />{m.movement_type === 'sale' && m.reference_type === 'online_order' ? 'Venta online' : m.movement_type === 'reservation_release' && m.reference_type === 'sale' ? 'Venta (reserva)' : (movementTypeLabels[m.movement_type] || m.movement_type)}</Badge></TableCell>
                  <TableCell>
