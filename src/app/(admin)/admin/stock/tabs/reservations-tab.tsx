...
export function ReservationsTab() {
  ...
  const confirmAddPayment = async () => {
    ...
    try {
      ...
      const autoReporter = createPrintReporter('reservation_ticket_print', {
      ...
      try {
        await printReservationPdf(buildReservationTicketData(chargedReservation), autoReporter.diag)
      } catch (printErr) {
        autoReporter.reportError(printErr)
  ...
  const handlePrintTicket = async (r: Reservation, mode: 'print' | 'download') => {
    ...
    const reporter = createPrintReporter('reservation_ticket_print', {
      call: 'manual',
      mode,
      reservation_number: r.reservation_number,
      reservation_id: r.id,
    ...
    try {
      if (mode === 'print') {
        await printReservationPdf(buildReservationTicketData(r), reporter.diag)
      } else {
