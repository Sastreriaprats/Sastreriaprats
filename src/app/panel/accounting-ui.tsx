...
export function MovementsTable({ rows }: { rows: MovementRow[] }) {
  ...
  const onDownload = async (saleId: string) => {
    setDl(saleId)
    try {
          ...
          {rows.length === 0 ? (
            ...
            <tr key={i} className="hover:bg-slate-50/60">
              ...
              <td className={`${TD} text-right`}>
                <button
                  onClick={() => onDownload(m.saleId)}
                  disabled={dl === m.saleId}
                ...
                >
                  {dl === m.saleId ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                </button>
              </td>
            </tr>
          ))}
