-- ============================================================
-- Migración 248 — rpc_create_sale: persistir sale_lines.tailoring_order_id por línea
--
-- CREATE OR REPLACE partiendo de la definición VIVA (migración 245). DIFF MÍNIMO:
-- ÚNICO cambio respecto a la 245 — en el INSERT INTO sale_lines (paso 5) se añade
-- la columna `tailoring_order_id` leída del jsonb por línea:
--     tailoring_order_id = NULLIF(l->>'tailoring_order_id','')::UUID
-- Si la línea NO trae ese campo (todas las de boutique) → NULL (comportamiento
-- actual, NULL = boutique real). Es el discriminador por línea (mig 247, Pieza 1)
-- para que los cobros de pedido del TPV salgan de boutique sin duplicar sastrería.
--
-- TODO LO DEMÁS ES BYTE-IDÉNTICO a la 245: cabecera (incl. el salesperson heredado
-- de la reserva, paso 3c), totales, pagos, serie CLP, caja, manual_transactions,
-- stock/reservas, retorno. Firma, SECURITY DEFINER, search_path y GRANTs idénticos.
-- ============================================================
...
BEGIN
  ...
  -- 5. INSERT sale_lines
  --    (248) Se añade tailoring_order_id por línea: si la línea es un cobro de
  --    pedido del TPV, el cliente lo envía y queda marcada como sastrería (sale de
  --    boutique en los informes). Las líneas de producto no lo traen → NULL.
