-- ============================================================
-- Migration 126: Tarjetas Regalo (Gift Cards)
--
-- 1. Añade 'gift_card' al CHECK de sale_type para distinguir
--    ventas de tarjetas regalo de ventas normales.
-- 2. Añade columnas voucher_kind y parent_voucher_id a vouchers
--    para distinguir vales de devolución, tarjetas regalo y
--    vales residuales generados al canjear con sobrante.
-- 3. Reemplaza rpc_create_sale para:
--    - Validar saldo y decrementar remaining_amount al usar voucher.
--    - Excluir pagos con voucher de manual_transactions
--      (evitar doble registro fiscal).
-- ============================================================

-- 1. sale_type ahora admite 'gift_card'
ALTER TABLE sales DROP CONSTRAINT IF EXISTS sales_sale_type_check;
ALTER TABLE sales ADD CONSTRAINT sales_sale_type_check
  CHECK (sale_type IN ('boutique', 'tailoring_deposit', 'tailoring_final', 'alteration', 'online', 'gift_card'));

-- 2. vouchers: voucher_kind + parent_voucher_id
ALTER TABLE vouchers
  ADD COLUMN IF NOT EXISTS voucher_kind TEXT DEFAULT 'return'
    CHECK (voucher_kind IN ('return', 'gift_card', 'residual'));

ALTER TABLE vouchers
  ADD COLUMN IF NOT EXISTS parent_voucher_id UUID
    REFERENCES vouchers(id) ON DELETE SET NULL;

-- Marcar los vales existentes (todos provienen de devoluciones).
UPDATE vouchers SET voucher_kind = 'return' WHERE voucher_kind IS NULL;

-- 3. Reemplazar rpc_create_sale
...
BEGIN
  ...
  -- 3b. Validar y decrementar saldo de cualquier voucher utilizado.
  --     Lo hacemos ANTES de crear la sale para fallar pronto si el vale no es válido.
  ...
  LOOP
    IF v_payment->>'payment_method' = 'voucher' AND NULLIF(v_payment->>'voucher_id', '') IS NOT NULL THEN
      ...
      UPDATE vouchers
         SET remaining_amount = remaining_amount - v_voucher_amt,
             status = CASE
               WHEN remaining_amount - v_voucher_amt <= 0 THEN 'used'::voucher_status
               ELSE 'partially_used'::voucher_status
             END,
             updated_at = v_now
       WHERE id = v_voucher_id;
    END IF;
