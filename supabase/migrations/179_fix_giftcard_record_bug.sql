-- ============================================================
-- Migración 179: fix bug "record v_giftcard is not assigned yet".
--
-- En 174/175 la cancelación de gift_card usaba:
--   IF v_sale.sale_type = 'gift_card' AND v_giftcard.id IS NOT NULL
-- En plpgsql esa expresión se compila como un único SELECT y plpgsql
-- materializa el parámetro v_giftcard.id ANTES de aplicar el AND. Para
-- una venta NO gift_card, v_giftcard nunca se asignó (su SELECT INTO está
-- dentro del cerrojo gift_card anterior), así que leer su campo lanza
-- SQLSTATE 55000 ("record v_giftcard is not assigned yet") -> excepción y
-- rollback en TODO borrado boutique. El cortocircuito booleano no salva
-- porque el fallo es al materializar el parámetro, no al evaluar el AND.
--
-- Fix: anidar el IF para que v_giftcard.id solo se lea cuando
-- sale_type='gift_card' (donde v_giftcard ya fue SELECT INTO'd en el
-- cerrojo de la cabecera). Resto de la función idéntico a la 175.
-- ============================================================
