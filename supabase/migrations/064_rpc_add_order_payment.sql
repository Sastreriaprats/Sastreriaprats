-- ============================================================
-- Migration 064: RPC add_order_payment
-- Consolida 7 queries secuenciales en 1 transacción atómica
...
BEGIN
  ...
  IF v_session_id IS NOT NULL THEN
    ...
    IF v_method_field IS NOT NULL THEN
      -- Get current values
      SELECT total_sales INTO v_current_total
      FROM cash_sessions WHERE id = v_session_id;

      IF v_method_field = 'total_cash_sales' THEN
        SELECT total_cash_sales INTO v_current_method FROM cash_sessions WHERE id = v_session_id;
      ELSIF v_method_field = 'total_card_sales' THEN
        SELECT total_card_sales INTO v_current_method FROM cash_sessions WHERE id = v_session_id;
      ELSE
        SELECT total_transfer_sales INTO v_current_method FROM cash_sessions WHERE id = v_session_id;
      END IF;

      UPDATE cash_sessions
      SET total_sales = COALESCE(v_current_total, 0) + p_amount
      WHERE id = v_session_id;

      -- Dynamic column update via CASE
      IF v_method_field = 'total_cash_sales' THEN
