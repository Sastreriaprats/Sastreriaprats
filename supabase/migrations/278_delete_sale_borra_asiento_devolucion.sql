-- 278_delete_sale_borra_asiento_devolucion.sql
--
-- Borrar del todo un ticket que tuvo devolución dejaba huérfano el asiento de
-- esa devolución: rpc_delete_sale_completely solo recolectaba los asientos con
-- reference_type='sale', y el de la devolución se graba como 'sale_return'.
-- El abono se quedaba en el Diario apuntando a una venta ya inexistente.
--
-- Hermano de la migración 277, que arregla lo mismo al ANULAR una devolución.
-- El asiento entra en `v_je_ids`, así que hereda tal cual el guard de periodo
-- contable cerrado y el borrado de sus líneas que ya hace la función.
--
-- Partida de la definición VIVA en producción. ÚNICO cambio: una rama más en
-- la UNION que recolecta los asientos. No toca ningún dato histórico.
...
BEGIN
  ...
  SELECT array_agg(DISTINCT s.je_id) INTO v_je_ids FROM (
    ...
    -- Asientos de las DEVOLUCIONES de esta venta (createSaleReturnJournalEntry
    -- los graba con reference_type='sale_return' y reference_id = la venta).
    -- Sin esto, borrar del todo un ticket que tuvo devolución dejaba su abono
    -- suelto en el Diario, apuntando a una venta que ya no existe.
    UNION SELECT je.id FROM journal_entries je WHERE je.reference_type = 'sale_return' AND je.reference_id = p_sale_id
  ) s;
