-- 260_delete_sale_reverts_reservations.sql
--
-- BUG (caso RSV-2026-0044 / TICK-2026-0335, 26-jun-2026): al eliminar por
-- completo una venta que había cumplido líneas de reserva, las líneas quedaban
-- en 'fulfilled' (con fulfilled_sale_id a NULL por la FK) y el recalc de
-- cabecera de la mig 200 ("recogida = pago total") mantenía la reserva como
-- payment_status='paid' con total_paid=total, SIN que existiera venta ni cobro
-- detrás. Resultado: deuda invisible (695 € en el caso real) y stock
-- restaurado pero sin re-bloquear (reserved no se re-incrementaba).
--
-- FIX: en rpc_delete_sale_completely, antes de borrar la venta:
--   1. Las líneas de reserva cumplidas por ESTA venta vuelven a 'active'
--      (fulfilled_at/fulfilled_sale_id a NULL) y se re-incrementa
--      stock_levels.reserved (quantity ya la restaura el bloque de reverso de
--      movimientos, que incluye el 'reservation_release' de la recogida).
--   2. La cabecera recalcula total_paid desde la realidad: pagos a cuenta
--      (product_reservation_payments) + líneas que sigan cumplidas por OTRAS
--      ventas (asunción mig 200: recogida = pago total), con tope en total.
--      El trigger de la mig 200 no baja nunca total_paid, por eso hace falta
--      este ajuste explícito.
--
-- Basada en la definición VIVA en prod (pg_get_functiondef, 2026-07-16), que
-- es la mig 211 + fixes previos. Cambios marcados con "MIG 260".
...
DECLARE
  ...
  v_je_ids           uuid[];
  v_mov              RECORD;
  v_resline          RECORD;      -- MIG 260
  v_res_ids          uuid[];      -- MIG 260
  v_n_res            int := 0;    -- MIG 260
  v_d_cash           numeric := 0;
  v_d_card           numeric := 0;
  v_d_bizum          numeric := 0;
...
BEGIN
  ...
  -- MIG 260: revertir líneas de reserva cumplidas por ESTA venta. Vuelven a
  -- 'active' y se re-bloquea el stock (reserved); la quantity ya la restauró
  -- el bloque anterior (el movimiento 'reservation_release' de la recogida
  -- tiene reference_type='sale'). Debe ir ANTES del DELETE de sales: la FK
  -- pondría fulfilled_sale_id a NULL y no encontraríamos las líneas.
  FOR v_resline IN
    SELECT l.id, l.reservation_id, l.product_variant_id, l.warehouse_id, l.quantity
    FROM product_reservation_lines l
    WHERE l.fulfilled_sale_id = p_sale_id AND l.status = 'fulfilled'
  LOOP
    UPDATE stock_levels
       SET reserved = reserved + v_resline.quantity,
           last_movement_at = now(),
           updated_at = now()
     WHERE product_variant_id = v_resline.product_variant_id
       AND warehouse_id = v_resline.warehouse_id;

    UPDATE product_reservation_lines
       SET status = 'active',
           fulfilled_at = NULL,
           fulfilled_sale_id = NULL,
           updated_at = now()
     WHERE id = v_resline.id;

    v_res_ids := array_append(v_res_ids, v_resline.reservation_id);
    v_n_res := v_n_res + 1;
  END LOOP;

  -- MIG 260: cabecera. El recalc de la mig 200 nunca BAJA total_paid, así que
  -- tras el reverso hay que recomputarlo: pagos a cuenta reales + líneas que
  -- sigan cumplidas por otras ventas (asunción "recogida = pago total"), con
  -- tope en el total. fulfilled_at solo se limpia si ya no queda línea cumplida.
  IF v_res_ids IS NOT NULL THEN
    UPDATE product_reservations r
       SET total_paid     = pay.paid,
           payment_status = CASE
             WHEN r.total > 0 AND pay.paid >= r.total THEN 'paid'
             WHEN pay.paid > 0 THEN 'partial'
             ELSE 'pending'
           END,
           fulfilled_at      = CASE WHEN pay.n_fulfilled = 0 THEN NULL ELSE r.fulfilled_at END,
           fulfilled_sale_id = CASE WHEN pay.n_fulfilled = 0 THEN NULL ELSE r.fulfilled_sale_id END,
           updated_at = now()
      FROM (
        SELECT r2.id,
               LEAST(r2.total, COALESCE(pp.paid, 0) + COALESCE(fl.fulfilled_total, 0)) AS paid,
               COALESCE(fl.n_fulfilled, 0) AS n_fulfilled
        FROM product_reservations r2
        LEFT JOIN (
          SELECT product_reservation_id, SUM(amount) AS paid
          FROM product_reservation_payments GROUP BY 1
        ) pp ON pp.product_reservation_id = r2.id
        LEFT JOIN (
          SELECT reservation_id, SUM(line_total) AS fulfilled_total, COUNT(*) AS n_fulfilled
          FROM product_reservation_lines WHERE status = 'fulfilled' GROUP BY 1
        ) fl ON fl.reservation_id = r2.id
        WHERE r2.id = ANY(v_res_ids)
      ) pay
     WHERE r.id = pay.id;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'deleted', jsonb_build_object(
      ...
      'reservation_lines_reverted', v_n_res,
