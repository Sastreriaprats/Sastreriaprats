...
-- (Idempotente: se puede ejecutar aunque ya existan tipos/tablas)
-- ==========================================
...
-- Añadir FK de sale_payments a vouchers (solo si no existe)
DO $$ BEGIN
...
-- Vincular boutique_alterations.sale_id (solo si no existe)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_alterations_sale') THEN
    ALTER TABLE boutique_alterations
      ADD CONSTRAINT fk_alterations_sale
      FOREIGN KEY (sale_id) REFERENCES sales(id) ON DELETE SET NULL;
  END IF;
EXCEPTION WHEN undefined_table THEN NULL;  -- por si boutique_alterations no existe aún
END $$;
