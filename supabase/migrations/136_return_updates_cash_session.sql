-- ============================================================
-- Migration 136: rpc_create_return — actualizar total_returns y total_sales
-- de la sesión de caja abierta de la tienda donde se realiza la devolución.
-- Antes: el campo cash_sessions.total_returns existía pero nunca se actualizaba.
-- Ahora: al crear una devolución se suma a total_returns y se resta de total_sales
-- de la sesión 'open' de la tienda. Si no hay sesión abierta, se ignora.
-- Resto del comportamiento idéntico a la 099.
-- ============================================================
...
BEGIN
  ...
  IF v_warehouse_id IS NOT NULL THEN
    ...
    LOOP
      ...
      ELSE
        -- Si no existía fila de stock, crearla con la cantidad devuelta
  ...
  -- Actualizar total_returns y total_sales de la sesión de caja abierta
  -- de la tienda donde se realiza la devolución. Si no hay sesión abierta,
  -- la devolución sigue su curso sin tocar caja (no es bloqueante).
