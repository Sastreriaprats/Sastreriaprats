-- ============================================================
-- Migración 253 — rpc_create_sale: venta con fecha RETROACTIVA
--
-- Caso real (Mónica): Roberto hizo ventas ayer y no las registró; Teresa las
-- mete hoy y necesita ponerles la fecha de ayer, y que cuenten en la caja de
-- ESE día, no en la de hoy.
--
-- CREATE OR REPLACE partiendo de la definición VIVA (migración 245). DIFF MÍNIMO:
--   1. p_sale acepta 'sale_date' (YYYY-MM-DD, opcional). Sin él o con la fecha
--      de hoy, comportamiento BYTE-IDÉNTICO al actual (fecha servidor).
--   2. Fecha futura → EXCEPTION.
--   3. Fecha pasada → se localiza la sesión de caja cuyo rango temporal cubre
--      esa fecha (MISMO algoritmo que rpc_add_order_payment, mig 135): primero
--      sesión 'open' con opened_at::date <= fecha, si no, sesión cerrada cuyo
--      [opened_at, closed_at] la englobe. Si ninguna la cubre → EXCEPTION con
--      mensaje claro (no se asigna en silencio a la caja de hoy).
--   4. La sesión RESUELTA (v_session_id) sustituye a p_sale->>'cash_session_id'
--      en: sales, totales de cash_sessions, manual_transactions y serie CLP.
--      Así la venta retro suma en los totales de la caja de SU día.
--   5. v_today pasa a ser la fecha de la venta → manual_transactions.date,
--      año de la serie TICK y caducidad de vales se evalúan a esa fecha.
--   6. sales.created_at y sale_payments.created_at se estampan con la fecha
--      retro (misma hora de reloj, N días atrás) → informes, contabilidad
--      (ops.ts filtra por sales.created_at) y asiento contable
--      (createSaleJournalEntry lee sales.created_at) caen en el día correcto.
--
-- Consciente y aceptado (igual que mig 135): añadir una venta a una sesión ya
-- CERRADA cambia sus totales esperados a posteriori; el arqueo contado de ese
-- día no se recalcula. Stock y sus movimientos siguen a NOW() (la salida física
-- del género ocurre al registrar).
-- ============================================================
...
BEGIN
  ...
  -- 6. Sale payments (253: created_at estampado con la fecha de la venta)
  INSERT INTO sale_payments (sale_id, payment_method, amount, reference, voucher_id, next_payment_date, created_at)
  SELECT
    v_sale_id,
