-- 274_fix_cobro_sale_double_count.sql
--
-- Cobrar la deuda de un TICKET A PLAZOS dentro de otro ticket sumaba el dinero
-- DOS VECES a la caja.
--
-- El TPV mete el pendiente como una línea del ticket nuevo. Al cobrar:
--   1) rpc_create_sale suma el total ÍNTEGRO del ticket a cash_sessions
--      (total_sales y el desglose por método), incluida esa línea;
--   2) el cliente llama después a addSalePayment -> rpc_add_sale_payment, que
--      vuelve a sumar el MISMO importe a cash_sessions.
-- Resultado: el arqueo espera el doble de lo que hay en el cajón y en
-- Movimientos aparecen dos apuntes por el mismo dinero.
--
-- Es el mismo fallo que la mig 271 arregló para los cobros de PEDIDO de
-- sastrería, que solo cubrió la rama con `tailoring_order_id`. Esta migración
-- lo hace simétrico para los cobros de venta, que el TPV marca con
-- `cobro_sale_id` (marcador que viaja solo en el JSON: NO es una columna de
-- sale_lines, y el INSERT de la RPC enumera columnas explícitas, así que se
-- ignora sin efectos).
--
-- Partida de la definición VIVA en producción (pg_get_functiondef), no de la
-- del repositorio. ÚNICO cambio funcional: el WHERE del bloque 2b.
-- No altera ningún dato histórico: los descuadres ya registrados se quedan como
-- están; a partir de aquí deja de duplicarse.
...
BEGIN
  ...
  -- 2b. (271/274) Base de las líneas de COBRO que YA entran en caja por su
  --     propio pago, para no sumarlas dos veces desde el ticket:
  --       · cobro de PEDIDO de sastrería (tailoring_order_id) -> addOrderPayment
  --         crea su tailoring_order_payment          (271)
  --       · cobro de TICKET a plazos (cobro_sale_id) -> addSalePayment llama a
  --         rpc_add_sale_payment, que suma el importe a cash_sessions  (274)
  --     Los cobros de ARREGLO no van aquí a propósito: markAlterationCharged no
  --     crea ningún pago, así que ese dinero entra en caja SOLO por el ticket.
  --     Se descuenta de lo que este ticket aporta a caja / al movimiento
  --     "Venta TPV". Se aplica el mismo descuento global que a v_total.
  SELECT COALESCE(SUM(
    ( (l->>'unit_price')::NUMERIC * (l->>'quantity')::INTEGER
      - (l->>'unit_price')::NUMERIC * (l->>'quantity')::INTEGER * COALESCE((l->>'discount_percentage')::NUMERIC, 0) / 100 )
  ...
  WHERE NULLIF(l->>'tailoring_order_id', '') IS NOT NULL
     OR NULLIF(l->>'cobro_sale_id', '') IS NOT NULL;
  ...
  SELECT
    ...
    -- 272: atribución por LÍNEA = dueño de la reserva de esa línea si la hay
    --      (product_reservations.employee_id, por reservation_id o
    --      reservation_line_id); en su defecto, el vendedor que cobra (cabecera).
    --      La reserva va a quien la hizo; lo complementario a quien está en caja.
    COALESCE(
      (SELECT r.employee_id FROM product_reservations r
        WHERE r.id = NULLIF(l->>'reservation_id', '')::UUID),
      (SELECT r2.employee_id FROM product_reservation_lines rl
         JOIN product_reservations r2 ON r2.id = rl.reservation_id
