-- ============================================================
-- Migración 271: doble conteo de cobros de pedido en tickets + vendedor
-- ============================================================
-- CONTEXTO / BUGS (reportados por Mónica, 24-jul-2026 — caso TICK-2026-0442)
--
-- BUG A — Doble conteo en caja/movimientos.
--   Cuando un pedido de sastrería se cobra DENTRO de un ticket del TPV, el dinero
--   entra una sola vez (una cobro de tarjeta), pero se registra DOS veces:
--     1) dentro del total del ticket, como línea al 0% ("Pedido sastrería - PIN…"
--        / "Cobro pendiente - PIN…"), que rpc_create_sale suma a
--        cash_sessions.total_*_sales y al movimiento "Venta TPV".
--     2) como tailoring_order_payment independiente (lo crea addOrderPayment tras
--        la venta), que rpc_add_order_payment suma OTRA VEZ a la caja y crea su
--        propio movimiento "Pago pedido".
--   Los informes de Contabilidad ya lo descontaban (helper pedido-cobro-lines.ts),
--   pero el CIERRE (total_sales/total_card_sales) y la LISTA de movimientos no.
--   Ejemplo: TICK-0442 = 7.055 € reales → la caja marcaba 13.765 €.
--
--   FIX A: rpc_create_sale netea la base de las líneas de cobro (las que llevan
--   tailoring_order_id) al sumar a cash_sessions.total_*_sales y al crear el
--   movimiento "Venta TPV". Así el ticket aporta a caja solo su parte propia
--   (boutique + reservas) y el cobro de pedido aporta vía su tailoring_order_payment
--   → cada euro una sola vez. NO cambia el ticket del cliente ni el IVA de la venta
--   (las líneas de cobro siguen igual; solo cambia lo que se refleja en caja).
--
-- BUG B — La reserva pisaba al vendedor que cobra.
--   Desde la mig 245 (mantenido en la 270), rpc_create_sale forzaba
--   salesperson = vendedor de la RESERVA por encima del vendedor que cobra. Si un
--   ticket incluía una línea de reserva de otro empleado, TODO el ticket (incluida
--   la boutique) se atribuía al dueño de la reserva. Caso real: reserva de Ismael,
--   venta cobrada por Elena → todo se fue a Ismael.
--
--   FIX B: manda el vendedor que cobra (el enviado por el TPV; en su defecto el
--   cajero). La reserva ya no pisa. Cabecera y todas las líneas heredan ese
--   vendedor. Coherente con "la comisión sigue al dinero que entra".
-- ============================================================
...
DECLARE
  ...
  v_sale_salesperson UUID;          -- 271: vendedor que COBRA (manda sobre la reserva)
  v_cobro_base     NUMERIC(12,2) := 0;  -- 271: base de líneas de cobro de pedido (netear en caja)
...
BEGIN
  ...
  -- 2b. (271) Base de las líneas de COBRO DE PEDIDO (llevan tailoring_order_id):
  --     el dinero de estas líneas entra en caja por su tailoring_order_payment
  --     (lo crea addOrderPayment tras la venta), así que NO debe volver a sumarse
  --     desde el ticket. Se descuenta de lo que este ticket aporta a caja / al
  --     movimiento "Venta TPV". Se aplica el mismo descuento global que a v_total.
  SELECT COALESCE(SUM(
    ( (l->>'unit_price')::NUMERIC * (l->>'quantity')::INTEGER
      - (l->>'unit_price')::NUMERIC * (l->>'quantity')::INTEGER * COALESCE((l->>'discount_percentage')::NUMERIC, 0) / 100 )
    * (1 - COALESCE((p_sale->>'discount_percentage')::NUMERIC, 0) / 100)
  ), 0)
  INTO v_cobro_base
  FROM jsonb_array_elements(p_lines) l
  WHERE NULLIF(l->>'tailoring_order_id', '') IS NOT NULL;
  ...
  -- 3d. (271) Vendedor efectivo = quien COBRA: salesperson enviado por el TPV; en
  --     su defecto, el cajero. La reserva YA NO pisa (antes, mig 245/270, el
  --     vendedor de la reserva mandaba sobre el cajero → todo el ticket se iba al
  --     dueño de la reserva). Se usa en la cabecera Y en cada línea.
  v_sale_salesperson := COALESCE(NULLIF(p_sale->>'salesperson_id', '')::UUID, p_user_id);
  ...
  --    (271) salesperson_id por línea = vendedor que cobra (cabecera). La reserva
  --    ya no atribuye la línea a su dueño.
  ...
  --    (271) Se netea la base de cobros de pedido (v_cobro_base): ese dinero entra
  --    por su tailoring_order_payment. total_sales aporta v_total - v_cobro_base;
  --    la parte por método se reduce en proporción al peso de cada método en el
  --    cobro (en el caso normal de un solo método, se descuenta todo de ese método).
  UPDATE cash_sessions SET
    total_sales          = COALESCE(total_sales, 0) + (v_total - v_cobro_base),
    total_cash_sales     = COALESCE(total_cash_sales, 0)
      ...
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * COALESCE((SELECT SUM((p->>'amount')::NUMERIC) FROM jsonb_array_elements(p_payments) p WHERE p->>'payment_method' = 'cash'), 0) / v_total_paid ELSE 0 END,
    total_card_sales     = COALESCE(total_card_sales, 0)
      ...
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * COALESCE((SELECT SUM((p->>'amount')::NUMERIC) FROM jsonb_array_elements(p_payments) p WHERE p->>'payment_method' = 'card'), 0) / v_total_paid ELSE 0 END,
    total_bizum_sales    = COALESCE(total_bizum_sales, 0)
      ...
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * COALESCE((SELECT SUM((p->>'amount')::NUMERIC) FROM jsonb_array_elements(p_payments) p WHERE p->>'payment_method' = 'bizum'), 0) / v_total_paid ELSE 0 END,
    total_transfer_sales = COALESCE(total_transfer_sales, 0)
      ...
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * COALESCE((SELECT SUM((p->>'amount')::NUMERIC) FROM jsonb_array_elements(p_payments) p WHERE p->>'payment_method' = 'transfer'), 0) / v_total_paid ELSE 0 END,
    total_voucher_sales  = COALESCE(total_voucher_sales, 0)
      ...
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * COALESCE((SELECT SUM((p->>'amount')::NUMERIC) FROM jsonb_array_elements(p_payments) p WHERE p->>'payment_method' = 'voucher'), 0) / v_total_paid ELSE 0 END
  ...
  --    (271) El importe se netea de la base de cobros de pedido (proporcional al
  --    peso del pago), para que la lista de movimientos no cuente dos veces (el
  --    "Pago pedido" ya lo crea addOrderPayment). Se omite el movimiento si tras
  --    netear queda ~0 (ticket que es SOLO cobros de pedido).
  ...
  CROSS JOIN LATERAL (
    SELECT (p->>'amount')::NUMERIC
      - CASE WHEN v_total_paid > 0 THEN v_cobro_base * (p->>'amount')::NUMERIC / v_total_paid ELSE 0 END
      AS net_amount
  ) n
  WHERE p->>'payment_method' <> 'voucher'
    AND n.net_amount > 0.005;
