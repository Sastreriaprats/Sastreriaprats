-- Categorías por tipo de producto: boutique, tejidos, complementos, servicios
-- y solo almacenes físicos para stock de productos

-- 1. Añadir tipo de producto a las categorías (sin CHECK primero para no fallar con NULLs)
ALTER TABLE product_categories
  ADD COLUMN IF NOT EXISTS product_type TEXT;

-- 2. Marcar todas las categorías existentes como boutique
UPDATE product_categories SET product_type = 'boutique' WHERE product_type IS NULL;

-- 2b. Añadir constraint (permite NULL; solo valida cuando hay valor)
DO $$
BEGIN
...
ON CONFLICT (slug) DO UPDATE SET product_type = EXCLUDED.product_type, name = EXCLUDED.name, path = EXCLUDED.path, sort_order = EXCLUDED.sort_order;

-- 5. Categorías para SERVICIOS (genérico; se pueden ampliar en configuración)
INSERT INTO product_categories (name, slug, level, path, sort_order, product_type) VALUES
...
CREATE INDEX IF NOT EXISTS idx_product_categories_product_type ON product_categories(product_type) WHERE product_type IS NOT NULL;
