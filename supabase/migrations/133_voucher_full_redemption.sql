-- ============================================================
-- Migración 133: Canje de vales transfiere TODO el saldo al residual
--
-- Antes (migración 126):
--   - Si se canjeaba un vale parcialmente, el original quedaba en
--     'partially_used' con remaining_amount = saldo restante.
--   - Adicionalmente, el JS creaba un vale residual con ese mismo
--     saldo restante.
--   - RESULTADO: saldo DUPLICADO. Original y residual canjeables a la vez.
--
-- Ahora:
--   - El vale original SIEMPRE queda en 'used' con remaining_amount = 0,
--     independientemente de si se canjeó total o parcialmente.
--   - El JS sigue creando el residual con el saldo restante (prev - amount).
--   - El saldo se "transfiere" del original al residual.
--
-- Solo cambia el bloque del UPDATE de vouchers en rpc_create_sale.
-- El resto de la función es idéntico a la migración 126.
-- ============================================================
