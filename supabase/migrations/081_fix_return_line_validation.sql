-- ============================================================
-- Migration 081: Validación robusta de líneas en rpc_create_return
-- Verifica que CADA línea del array p_line_ids pertenece a la venta
-- ============================================================
...
BEGIN
  ...
  -- 2. Validar que TODAS las líneas pertenecen a esta venta
  ...
  IF v_matching_count != array_length(p_line_ids, 1) THEN
    RAISE EXCEPTION 'Una o más líneas no pertenecen a la venta original';
  END IF;

  -- 3. Calculate total returned from selected lines
  ...
  -- 4. Create voucher if return_type = 'voucher'
  ...
  -- 6. Mark selected lines as returned
  UPDATE sale_lines
  SET is_returned = TRUE
  WHERE id = ANY(p_line_ids);

  -- 7. Check if all non-returned lines are now returned
  SELECT NOT EXISTS (
  ...
  -- 9. Restore stock for returned lines
...
-- ============================================================
-- NOTA sobre RLS de client_companies (migración 076):
--
-- Las políticas RLS permiten CRUD a cualquier usuario "authenticated".
-- Esto es seguro en la práctica porque:
-- 1. El componente que accede (client-companies-tab.tsx) está dentro
--    de /admin/clientes/[id]/, protegido por middleware que requiere
--    role admin/vendedor.
-- 2. El browser client (createClient) hereda la sesión del usuario
--    que ya pasó el middleware auth.
-- 3. Un usuario público (sin rol) no puede llegar a esa ruta.
--
-- Si en el futuro se expone client_companies a la web pública,
-- se deberá añadir filtro por client_id = auth.uid() o similar.
-- ============================================================
