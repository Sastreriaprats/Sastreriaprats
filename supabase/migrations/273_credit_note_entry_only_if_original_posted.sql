-- ============================================================
-- Migración 273: la rectificativa solo crea asiento si la factura ORIGINAL
-- tenía asiento propio.
--
-- PROBLEMA. `rpc_create_credit_note` (mig 192, + fix 193 en line_total) creaba
-- SIEMPRE la contrapartida D 708 + D 477 / H 430. Pero las facturas emitidas
-- desde un ticket de TPV, un pedido de sastrería o una reserva NO tienen asiento
-- propio (`invoices.journal_entry_id` IS NULL): el ingreso y el IVA los aporta la
-- venta / el cobro, y la factura es solo el documento. Abonar una de esas
-- facturas restaba en el diario un ingreso que la factura nunca aportó:
--   · la venta del TPV sigue viva (nadie la toca) → el ingreso sigue en A,
--   · y encima entraba un 708 por la base y un 477 por la cuota,
--   → ingresos y IVA repercutido del trimestre restados de más.
-- En prod (1-ago-2026) solo había una rectificativa emitida, R2026-0001, sobre
-- una factura manual CON asiento, así que el descuadre no llegó a producirse.
--
-- ARREGLO. El bloque F pasa a ser condicional: se crea la contrapartida
-- únicamente si `v_inv.journal_entry_id IS NOT NULL`. Es el mismo criterio que
-- ya aplica la anulación (`reverseInvoiceJournalEntry` devuelve `skipped` sin
-- asiento original): el diario solo espeja lo que realmente hubo.
-- Cuando no hay asiento, la R queda como documento fiscal sin apunte —igual que
-- la factura que rectifica— y `journal_entry_id` vuelve NULL en el JSON.
--
-- El resto de la función es idéntico a la definición VIVA en producción
-- (pg_get_functiondef, verificado antes de escribir esta migración).
--
...
BEGIN
  ...
  -- SOLO si la factura original tenía asiento propio. Las facturas de ticket,
  -- pedido de sastrería o reserva no lo tienen (el ingreso lo aporta la venta o
  -- el cobro): crear aquí un 708 restaría un ingreso que esta factura nunca
  -- aportó al diario. Mismo criterio que la anulación.
  IF v_inv.journal_entry_id IS NOT NULL THEN
    v_try := 0;
    LOOP
      v_try := v_try + 1;
  ...
  RETURN jsonb_build_object(
    ...
    'total',               -v_total,
    'journal_entry_id',    v_entry_id,   -- NULL si la original no tenía asiento
