-- ============================================================
-- Migration 141: rpc_create_return — solo afectar cash_sessions
-- cuando la devolución es realmente en efectivo.
--
-- Bug que corrige (mig. 136/138): se actualizaba total_returns y
-- se restaba de total_sales para CUALQUIER devolución (incluidos
-- vale y exchange), pero esas devoluciones no sacan dinero del
-- cajón. El cierre de caja quedaba con "Efectivo esperado" negativo
-- y era imposible cuadrar.
--
-- Comportamiento nuevo:
--   - return_type = 'cash'    → actualiza total_returns/total_sales
--   - return_type = 'voucher' → NO toca cash_sessions (el vale se
--                               registra en la tabla vouchers)
--   - return_type = 'exchange'→ NO toca cash_sessions (el dinero
--                               se mueve cuando se cobra la nueva
--                               venta del cambio)
--
-- Resto del comportamiento idéntico a la 138.
-- ============================================================
...
BEGIN
  ...
  -- Vale (voucher) y cambio (exchange) NO mueven dinero del cajón en este
  -- momento: el vale es una obligación pendiente y el exchange se cobra
  -- al crear la nueva venta del cambio.
