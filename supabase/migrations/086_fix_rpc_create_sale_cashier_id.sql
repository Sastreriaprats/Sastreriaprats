-- ============================================================
-- Migration 086: Fix rpc_create_sale — cashier_id → salesperson_id
--
-- La migración 073 reescribió la función y usó "cashier_id"
-- pero la tabla sales tiene "salesperson_id".
-- Restauramos la función completa de la 062 con category 'boutique'.
-- ============================================================
...
BEGIN
  ...
  v_sale_discount := v_subtotal * COALESCE((p_sale->>'discount_percentage')::NUMERIC, 0) / 100;
  v_taxable_total := v_subtotal - v_sale_discount;
  v_tax_amount    := v_taxable_total * 0.21;
  v_total         := v_taxable_total + v_tax_amount;
  ...
  SELECT
    ...
    (
      ...
      - (l->>'unit_price')::NUMERIC * (l->>'quantity')::INTEGER
        * COALESCE((l->>'discount_percentage')::NUMERIC, 0) / 100
    ) * (1 + COALESCE((l->>'tax_rate')::NUMERIC, 21) / 100),
