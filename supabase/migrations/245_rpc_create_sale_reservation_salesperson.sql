-- ============================================================
-- Migración 245 — rpc_create_sale: la cabecera hereda el vendedor de la reserva
--
-- CREATE OR REPLACE partiendo de la definición VIVA (migración 235). DIFF MÍNIMO:
-- únicos cambios respecto a la 235 (todo lo demás —líneas, stock, pagos, caja,
-- journal, serie CLP, vouchers, retorno— es BYTE-IDÉNTICO):
--   1. DECLARE: nueva variable  v_reservation_salesperson UUID;
--   2. Nuevo paso 3c (antes del INSERT INTO sales): calcula el employee_id de la
--      reserva MÁS ANTIGUA entre las líneas que traen reservation_id (con LEFT JOIN
--      a product_reservation_lines como defensa por si llega reservation_line_id
--      sin reservation_id).
--   3. INSERT INTO sales: salesperson_id pasa a
--        COALESCE(v_reservation_salesperson, <selector TPV>, p_user_id)
--      → Opción A: la RESERVA manda siempre; el selector del TPV solo gobierna
--        ventas DIRECTAS (sin reserva); el cajero es el último recurso.
--
-- Regla confirmada (Ismael): la comisión de una venta-de-reserva va SIEMPRE a quien
-- reservó; el ticket ENTERO a la "primera acción" (la reserva) → solo cabecera, no
-- se parte por líneas. El informe de comisiones (getEmployeeCommissions) lee la
-- cabecera, así que con esto la comisión queda reatribuida.
-- ============================================================
