...
-- Vendedor asignado a reservas y propagación a líneas de venta
-- ==========================================
-- Cambios:
--   1. Añade product_reservations.employee_id (vendedor que crea la reserva)
--   2. Añade sale_lines.salesperson_id (vendedor por línea, para comisiones)
--   3. rpc_create_reservation: acepta y guarda employee_id
--   4. rpc_create_sale:
--        - INSERT sale_lines con salesperson_id heredado de reservation_line
--          cuando la línea viene con reservation_line_id explícito.
--        - Tras auto-fulfill FIFO, actualiza salesperson_id de la sale_line
--          con el employee_id de la PRIMERA reserva consumida.
--        - Usa WITH ORDINALITY para tener sort_order determinista
--          y poder localizar la sale_line correcta en el UPDATE FIFO.
...
CREATE INDEX IF NOT EXISTS idx_product_reservations_employee
  ON product_reservations(employee_id);

ALTER TABLE sale_lines
  ADD COLUMN IF NOT EXISTS salesperson_id UUID REFERENCES profiles(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_sale_lines_salesperson
  ON sale_lines(salesperson_id);

-- Backfill histórico: heredar salesperson de la cabecera para las líneas sin asignar
...
-- ─────────────────────────────────────────────────────────────
-- 2. rpc_create_reservation (con employee_id)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 3. rpc_create_sale (propaga vendedor reserva → sale_line)
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- 5. INSERT sale_lines
  --    sort_order usa WITH ORDINALITY (1..N) de forma determinista
  --    para poder localizar la línea en el UPDATE del FIFO más abajo.
  --    salesperson_id: si la línea trae reservation_line_id explícito,
  --    hereda el vendedor de la reserva; si no, el de la cabecera.
  INSERT INTO sale_lines (
    ...
    cost_price, sort_order, reservation_id, reservation_line_id, salesperson_id
  )
  SELECT
    v_sale_id,
  ...
  IF v_warehouse_id IS NOT NULL THEN
    FOR v_line_row IN
      SELECT l, ord::INTEGER AS idx
        FROM jsonb_array_elements(p_lines) WITH ORDINALITY AS t(l, ord)
    LOOP
      IF v_line_row.l->>'product_variant_id' IS NULL OR v_line_row.l->>'product_variant_id' = '' THEN
        CONTINUE;
      END IF;

      v_variant_id  := (v_line_row.l->>'product_variant_id')::UUID;
      v_line_qty    := (v_line_row.l->>'quantity')::INTEGER;
      v_res_line_id := NULLIF(v_line_row.l->>'reservation_line_id', '')::UUID;
      v_line_idx    := v_line_row.idx;

      -- 9.A — Recogida explícita de una línea de reserva
      ...
      -- 9.B — Línea normal: auto-fulfill FIFO por cliente
      v_remaining           := v_line_qty;
      v_first_fifo_employee := NULL;

      IF v_client_id IS NOT NULL AND v_remaining > 0 THEN
        FOR v_fifo_line IN
          SELECT l.id AS line_id, l.quantity AS line_qty, r.reservation_number AS r_num, r.employee_id AS r_employee
            FROM product_reservation_lines l
            JOIN product_reservations r ON r.id = l.reservation_id
        ...
        LOOP
          ...
          IF v_first_fifo_employee IS NULL AND v_fifo_line.r_employee IS NOT NULL THEN
            v_first_fifo_employee := v_fifo_line.r_employee;
          END IF;
        ...
        -- Si la línea de venta ha consumido reservas, heredar el vendedor
        -- de la primera reserva consumida (FIFO) sobre esta sale_line.
        IF v_first_fifo_employee IS NOT NULL THEN
          UPDATE sale_lines
             SET salesperson_id = v_first_fifo_employee
           WHERE sale_id    = v_sale_id
             AND sort_order = v_line_idx;
        END IF;
      END IF;
        ...
        IF v_stock_rec.id IS NOT NULL THEN
          ...
          IF v_available < v_remaining THEN
            RAISE EXCEPTION 'Stock insuficiente (reservado para otros clientes). Producto: %',
              COALESCE(v_line_row.l->>'description', v_variant_id::TEXT);
          END IF;
