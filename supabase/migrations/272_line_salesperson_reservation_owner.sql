-- ============================================================
-- Migración 272: atribución por línea (reserva → su dueño; resto → quien cobra)
-- ============================================================
-- La mig 271 (arreglo del vendedor) se pasó: mandó TODAS las líneas al cajero,
-- incluida la de la RESERVA. Regla correcta de David (24-jul-2026): el importe de
-- la RESERVA va SIEMPRE al vendedor que la hizo (product_reservations.employee_id);
-- las líneas COMPLEMENTARIAS del mismo ticket van a quien está en caja (quien
-- cobra). Es atribución POR LÍNEA — la definición canónica de "facturado por
-- empleado" (employee-billing.ts) ya la espera ("a quien vendió o reservó"). La
-- cabecera (sales.salesperson_id) sigue siendo el vendedor que cobra (mig 271).
-- Solo cambia la salesperson_id por línea del INSERT de sale_lines; el resto (el
-- neteo de doble conteo de la 271) queda intacto.
-- ============================================================
